...
###########################
# Group Members:
# By: MANYA PUMA Jorge Luis
# Partner: ZANNETTOS Lambros
###########################
...
def point_add(a, b, p, x0, y0, x1, y1):
    ...
    # Check if (x0, y0) point is infinite
    if ((x0 is None) and (y0 is None)):
        return (x1, y1)
    # Check if (x1, y1) point is infinite
    if ((x1 is None) and (y1 is None)):
        return(x0, y0)
    if (x0 is x1) or not (is_point_on_curve(a, b, p, x0, y0) and is_point_on_curve(a, b, p, x1, y1)):
        return(None, None)
...
def point_scalar_multiplication_double_and_add(a, b, p, x, y, scalar):
    ...
    for i in range(scalar.num_bits()):
        p1, p2 = P
        if scalar.is_bit_set(i): ## ADD YOUR CODE HERE
            q1, q2 = Q
...
def point_scalar_multiplication_montgomerry_ladder(a, b, p, x, y, scalar):
    ...
    for i in reversed(range(0,scalar.num_bits())):
        ...
        if not scalar.is_bit_set(i):
            ...
            R1 = point_double(a, b, p, q1, q2)  ## ADD YOUR CODE HERE
...
def dh_decrypt(priv, iv, ciphertext, tag, pub_A, aliceVer = None):
    ...
    shared_key, y = pub_A.pt_mul(priv).get_affine()
    shared_key = sha256(shared_key.repr()).digest()[:32]
...
#  The test covers wrong ciphertext, wrong tag, wrong public key, wrong private key
...
def test_decrypt():
    ...
    message = u"Hello World!"
    iv, ciphertext, tag, pub_Bob = dh_encrypt(pub_Alice, message)
    m = dh_decrypt(priv_Alice, iv, ciphertext, tag, pub_Bob)
    assert m == message
...
def test_fails():
    ...
    from os import urandom
    G, priv_Alice, pub_Alice = dh_get_key()
    message = u"Tests that fail"
    iv, ciphertext, tag, pub_Bob = dh_encrypt(pub_Alice, message)

    #Wrong ciphertext test
    with raises(Exception) as excinfo:
        dh_decrypt(priv_Alice, iv, urandom(len(ciphertext)), tag, pub_Bob)
    assert 'decryption failed' in str(excinfo.value)

    #Wrong tag  test
    with raises(Exception) as excinfo:
        dh_decrypt(priv_Alice, iv, ciphertext, urandom(len(tag)), pub_Bob)
    assert 'decryption failed' in str(excinfo.value)

    #Wrong Keys tests
    G, priv_wrong, pub_wrong = dh_get_key()
    #Wrong Public Key test
    with raises(Exception) as excinfo:
        dh_decrypt(priv_Alice, iv, ciphertext, urandom(len(tag)), pub_wrong)
    assert 'decryption failed' in str(excinfo.value)

    #Wrong Private Key test
    with raises(Exception) as excinfo:
        dh_decrypt(priv_wrong, iv, ciphertext, urandom(len(tag)), pub_Bob)
    assert 'decryption failed' in str(excinfo.value)
#####################################################
